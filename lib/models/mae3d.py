...
def patchify_image(x, patch_size):
    ...
    grid_size = (H // patch_size[0], W // patch_size[1], D // patch_size[2])#高宽深对应的patch个数grid_size
...
class MAE3D(nn.Module):
    ...
    def __init__(self, 
        ...
        super().__init__()
        self.args = args
        input_size = to_3tuple(args.input_size) #(96,96,96)
        patch_size = to_3tuple(args.patch_size)#(16,16,16)
        ...
        self.mask_token = nn.Parameter(torch.zeros(1, 1, args.decoder_embed_dim))#mask_token 初始值
